...
examples_path = attribute(:examples_path,
  default: File.join(File.dirname(__FILE__), "..", "fixtures"),
  description: "Path to examples")

if command("#{hab} pkg path $HAB_ORIGIN/scaffolding-node").exit_status != 0
  puts "#{ENV['HAB_ORIGIN']}/scaffolding-node not found. Building from /src/scaffolding-node..."
  command("#{build} /src/scaffolding-node").result
  puts command("#{hab} pkg path $HAB_ORIGIN/scaffolding-node").stdout
end
